...
- [ ] `.env.local` を作成して `NEXT_PUBLIC_API_URL=http://localhost:3000` を書く
...
shadcn/ui のコンポーネント（Card・Checkbox・Dialog・Button・Input）を積極的に使う。  
...
- [ ] Server Component として `fetch` で習慣・記録を取得する（`'use client'` を書かない）
- [ ] 過去28日分の日付配列を作る
